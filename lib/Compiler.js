...
class Compiler {
	...
	// 这个run 方法在外部会被 compiler 实例调用 compiler.run()
	/**
...
${other}`);
	...
	// compiler 直译就是编译的意思，它里面包含的就是 webpack 打包流程
	// 在这之前我们其实就是做了：命令行参数的处理 options + compiler 的声明
	// beforeCompile compile make finishMake afterCompile  这几个是webpack 的核心生命周期
	/**
	...
	compile(callback) {
		...
		this.hooks.beforeCompile.callAsync(params, err => {
			...
			// compiler 与 compilation 的区别在这里，
			// compiler 会比 compilation 大一点；
			// compiler 是启动webpack编译时创建的，每次修改文件改动时，webpack重新watch编译时 创建一个 compilation；
			// 你只要 npm run build 就会创建一个 compiler， 之后，开启watch模式时，这个 compiler 里面会有 很多次 compilation，
			// 每次文件改动打包 都需要一个compilation，

			// 在 beforeCompile 里面声明了一个 compilation
			const compilation = this.newCompilation(params);

			const logger = compilation.getLogger("webpack.Compiler");

			logger.time("make hook");
			// 真正 读取一个模块文件，然后转换，然后 写入 的操作 在make
